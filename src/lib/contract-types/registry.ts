...
const NDA_PROFILE: ContractTypeProfile = {
  ...
  sections: [
    ...
    { id: "termination", visible: true, required: false, emptyStateMessage: "Keine ordentliche Kündigung — bei NDAs üblich, da Verschwiegenheitspflicht laufzeitunabhängig" },
  ...
  marketStandardClauses: [
    ...
    { id: "non-solicitation", name: "Non-Solicitation (Mitarbeiter-Abwerbeverbot)", category: "should-have", description: "Schutz vor Abwerbung von Schluesselmitarbeitern während und nach Verhandlungen.", fallbackTemplate: "Die Parteien verpflichten sich, während der Laufzeit dieser Vereinbarung und für einen Zeitraum von zwoelf (12) Monaten danach keine Mitarbeiter der jeweils anderen Partei aktiv abzuwerben." },
    { id: "residuals-clause", name: "Residuals Clause", category: "should-have", description: "Regelung ob unbewusst behaltenes Erinnerungswissen nach NDA-Ende verwendet werden darf.", fallbackTemplate: "Unabhaengig von den vorgenannten Bestimmungen duerfen die Parteien Erinnerungswissen ihrer Mitarbeiter (Residuals), das ohne bewusstes Zurückgreifen auf die vertraulichen Informationen behalten wurde, frei verwenden." },
    { id: "return-of-information", name: "Rückgabe / Vernichtung", category: "must-have", description: "Pflicht zur Rückgabe oder Vernichtung vertraulicher Unterlagen nach Vertragsende.", fallbackTemplate: "Nach Beendigung dieser Vereinbarung wird jede Partei alle erhaltenen vertraulichen Informationen, einschliesslich saemtlicher Kopien, unverzueglich zurückgeben oder nachweislich vernichten und dies schriftlich bestaetigen." }
  ],
...
const SAAS_PROFILE: ContractTypeProfile = {
  ...
  sections: [
    ...
    { id: "termination", visible: true, required: true, emptyStateMessage: "Kündigungsfrist fehlt — Marktstandard: 3 Monate zum Vertragsende" },
    ...
    { id: "indemnification", visible: true, required: true, emptyStateMessage: "Freistellungsregelung fehlt (insb. für IP-Verletzungen)" },
    ...
    { id: "service-levels-sla", visible: true, required: true, emptyStateMessage: "SLA fehlt — Verfügbarkeit, Reaktionszeiten, Support nicht definiert" },
    ...
    { id: "warranties", visible: true, required: false, emptyStateMessage: "Gewährleistung fehlt — Standard: Funktionalitaet gemäß Dokumentation" },
    { id: "governing-law", visible: true, required: true, emptyStateMessage: "Anwendbares Recht und Gerichtsstand fehlen" },
    { id: "compliance-clauses", visible: true, required: true, emptyStateMessage: "Compliance-Klauseln (DSGVO, Audit-Recht) fehlen" }
  ],
  marketStandardClauses: [
    { id: "avv-art28", name: "Auftragsverarbeitungsvertrag (Art. 28 DSGVO)", category: "must-have", description: "Bei Verarbeitung personenbezogener Daten zwingend erforderlich. Muss alle Pflichtinhalte aus Art. 28 Abs. 3 DSGVO enthalten.", fallbackTemplate: "Sofern der Anbieter im Rahmen der Leistungserbringung personenbezogene Daten des Kunden verarbeitet, schliessen die Parteien einen Auftragsverarbeitungsvertrag gemäß Art. 28 DSGVO ab, der diesem Vertrag als Anlage AVV beigefuegt ist und integraler Vertragsbestandteil wird.", bgbReference: "Art. 28 DSGVO" },
    { id: "sla-uptime", name: "SLA Verfügbarkeit (99,5% Marktstandard)", category: "must-have", description: "Garantierte monatliche Verfügbarkeit mit Service Credits bei Unterschreitung.", fallbackTemplate: "Der Anbieter garantiert eine monatliche Verfügbarkeit von mindestens 99,5% (gemessen über den Kalendermonat, ausgenommen geplante Wartungsfenster). Bei Unterschreitung erhält der Kunde Service Credits gemäß Anlage SLA." },
    ...
    { id: "audit-right", name: "Audit-Recht des Kunden", category: "should-have", description: "Recht zur Prüfung der Compliance des Anbieters mit vereinbarten Sicherheits- und Datenschutzstandards.", fallbackTemplate: "Der Kunde ist berechtigt, einmal jährlich nach Vorankündigung und während üblicher Geschäftszeiten ein Audit der vereinbarten Sicherheits- und Datenschutzmaßnahmen durchzuführen. Alternativ akzeptiert der Kunde aktuelle Zertifikate (ISO 27001, SOC 2 Typ II)." },
    { id: "ai-transparency-eu-ai-act", name: "KI-Transparenz (EU AI Act Art. 50)", category: "must-have", description: "Pflicht zur Offenlegung KI-basierter Entscheidungen (ab August 2026 verpflichtend).", fallbackTemplate: "Sofern der Anbieter KI-Systeme zur automatisierten Entscheidungsfindung einsetzt, informiert er den Kunden transparent über Art, Umfang und Zweck dieser Systeme gemäß Art. 50 EU AI Act. Der Anbieter stellt eine schriftliche Erklärung zur EU AI Act Konformitaet bereit." },
    { id: "subprocessor-list", name: "Unterauftragsverarbeiter-Verzeichnis", category: "must-have", description: "Liste aller Subprocessors mit Änderungsbenachrichtigung 30 Tage im Voraus.", fallbackTemplate: "Der Anbieter fuehrt ein Verzeichnis aller eingesetzten Unterauftragsverarbeiter und stellt es dem Kunden auf Anfrage zur Verfuegung. Änderungen werden mindestens 30 Tage im Voraus angekündigt; der Kunde hat ein Widerspruchsrecht." }
  ],
  ...
  wording: {
    ...
    terminationLabel: "Kündigungsregelung",
...
const AVV_PROFILE: ContractTypeProfile = {
  ...
  sections: [
    ...
    { id: "compliance-clauses", visible: true, required: true, emptyStateMessage: "Technisch-organisatorische Maßnahmen (TOM) fehlen" },
  ...
  marketStandardClauses: [
    { id: "tom-art32", name: "Technisch-organisatorische Maßnahmen (Art. 32 DSGVO)", category: "must-have", description: "Konkrete Beschreibung der Sicherheitsmassnahmen — nicht nur Verweis auf 'Stand der Technik'.", fallbackTemplate: "Der Auftragsverarbeiter ergreift die in Anlage TOM beschriebenen technischen und organisatorischen Maßnahmen gemäß Art. 32 DSGVO, einschliesslich Verschlüsselung im Transport (TLS 1.2+) und at-rest (AES-256), Zugriffskontrolle (RBAC), Pseudonymisierung wo möglich, regelmaessige Sicherheitsaudits und Incident-Response-Plaene.", bgbReference: "Art. 32 DSGVO" },
    { id: "subprocessor-art28", name: "Unterauftragsverarbeiter-Regelung (Art. 28 DSGVO)", category: "must-have", description: "Vorherige schriftliche Zustimmung oder allgemeine Genehmigung mit Widerspruchsrecht.", fallbackTemplate: "Der Einsatz von Unterauftragsverarbeitern bedarf der vorherigen schriftlichen Genehmigung des Verantwortlichen. Eine Liste der genehmigten Unterauftragsverarbeiter ist als Anlage Subprocessors beigefuegt; Änderungen werden 30 Tage im Voraus mitgeteilt mit Widerspruchsrecht." },
    { id: "data-subject-rights", name: "Unterstuetzung bei Betroffenenrechten", category: "must-have", description: "Pflicht zur Unterstuetzung bei Auskunft, Löschung, Datenuebertragbarkeit.", fallbackTemplate: "Der Auftragsverarbeiter unterstützt den Verantwortlichen bei der Erfüllung der Rechte betroffener Personen (Art. 12-22 DSGVO) durch geeignete technische und organisatorische Maßnahmen, soweit möglich, und antwortet auf Anfragen innerhalb von 7 Werktagen." },
    { id: "breach-notification", name: "Meldung von Datenschutzverletzungen (Art. 33 DSGVO)", category: "must-have", description: "Meldung an Verantwortlichen innerhalb von 24-72 Stunden nach Kenntnisnahme.", fallbackTemplate: "Der Auftragsverarbeiter meldet dem Verantwortlichen jede Verletzung des Schutzes personenbezogener Daten unverzueglich, spaetestens innerhalb von 24 Stunden nach Kenntnisnahme, und unterstützt bei der Erfüllung der Meldepflicht gemäß Art. 33 DSGVO." },
    { id: "international-transfer-sccs", name: "Internationale Datenuebermittlung (SCCs)", category: "must-have", description: "Bei Drittlandtransfer: aktuelle Standardvertragsklauseln (2021/914) als Anlage.", fallbackTemplate: "Sofern personenbezogene Daten in ein Drittland uebermittelt werden, das kein angemessenes Datenschutzniveau bietet, schliessen die Parteien die EU-Standardvertragsklauseln 2021/914 in der jeweils aktuellen Fassung als Anlage SCCs ab und treffen ergänzende Schutzmassnahmen gemäß EDPB Recommendations 01/2020." }
  ],
...
export const CONTRACT_TYPE_REGISTRY: Record<ContractTypeId, ContractTypeProfile> = {
  ...
  "saas": SAAS_PROFILE,
  "avv": AVV_PROFILE,
  "lieferantenvertrag": { ...SUPPLIER_EN_PROFILE, id: "lieferantenvertrag", name: "Lieferantenvertrag", shortName: "Lieferant", jurisdiction: "DE", wording: { termLabel: "Vertragslaufzeit", noticeLabel: "Kündigungsfrist", terminationLabel: "Kündigung", partyALabel: "Auftraggeber", partyBLabel: "Lieferant" } },
  "rahmenvertrag": { ...SUPPLIER_EN_PROFILE, id: "rahmenvertrag", name: "Rahmenvertrag", shortName: "Rahmenvertrag", jurisdiction: "DE", wording: { termLabel: "Rahmenvertragslaufzeit", noticeLabel: "Kündigungsfrist", terminationLabel: "Kündigung", partyALabel: "Auftraggeber", partyBLabel: "Auftragnehmer" } },
