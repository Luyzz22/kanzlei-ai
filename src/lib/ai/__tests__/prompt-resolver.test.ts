...
test("Registry-Resolver liefert stabile Keys und Versionen ohne DB", async () => {
  ...
  assert.ok(ext.text.includes("Strukturierte Extraktion"))
  assert.ok(!ext.text.includes(norm), "Instructions enthalten keinen Vertragstext (Hotfix 9)")
  ...
  assert.ok(!risk.text.includes(norm), "Risk-Instructions enthalten keinen Vertragstext (Hotfix 9)")
})
