...
export default function LoginPage() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setError(null)
    // TODO: Credentials-Login mit NextAuth einbinden
        ...
        <div className="w-full">
          <section className="rounded-2xl border border-slate-800 bg-slate-900 p-8 shadow-xl sm:p-10">
            <div className="mb-8 text-center">
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-amber-400">Kanzlei-AI</p>
              <h1 className="mt-4 text-3xl font-semibold text-slate-50">Willkommen zurück</h1>
              <p className="mt-2 text-sm text-slate-300">Melden Sie sich an, um Kanzlei-AI zu nutzen.</p>
            </div>
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              <div className="rounded-lg border border-dashed border-slate-700 px-4 py-5 text-center text-sm text-slate-400">
                {/* TODO: SSO-Login einbinden */}
              </div>
            </form>
          </section>

          <footer className="mt-6 text-center text-sm text-slate-300">
            <Link href="/password-reset" className="font-medium text-slate-100 underline-offset-4 hover:underline">
              Passwort vergessen?
            </Link>
            <p className="mt-3">
              Noch kein Konto?{" "}
              <Link href="/register" className="font-semibold text-amber-300 underline-offset-4 hover:underline">
                Jetzt registrieren
              </Link>
            </p>
          </footer>
