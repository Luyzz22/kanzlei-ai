...
export function ReleaseNotesWidget({ count = 4 }: { count?: number }) {
  ...
  return (
    ...
    >
      ...
      <header
        className="flex items-center justify-between px-5 py-4 border-b border-stone-200/60 cursor-pointer select-none hover:bg-stone-50/40 transition-colors rounded-t-2xl"
        ...
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="text-[11px] text-stone-500 mt-0.5">
              {isOpen ? "Releases und Compliance-Updates" : `${RELEASE_NOTES[0]?.version} — ${RELEASE_NOTES[0]?.title}`}
            </p>
          </div>
        </div>
